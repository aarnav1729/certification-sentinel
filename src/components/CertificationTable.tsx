...
export const CertificationTable = ({
  ...
  const filteredCerts = certifications.filter((cert) => {
    ...
    return (
      ...
      cert.rNo.toLowerCase().includes(query) ||
            ...
            {filteredCerts.length === 0 ? (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-12 text-muted-foreground">
                  {searchQuery ? 'No certifications match your search' : 'No certifications yet'}
                </TableCell>
              </TableRow>
            ) : (
              filteredCerts.map((cert, index) => (
                ...
                >
                  ...
                  <TableCell className="font-mono text-sm">{cert.rNo}</TableCell>
                  <TableCell><TypeBadge type={cert.type} /></TableCell>
                  <TableCell><StatusBadge status={cert.status} /></TableCell>
                  <TableCell>{formatDate(cert.validityFrom)}</TableCell>
                  <TableCell>{formatDate(cert.validityUpto)}</TableCell>
                  <TableCell>
                    {cert.validityUpto ? (
                      <ExpiryBadge validityUpto={cert.validityUpto} />
                    ) : (
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this certification and stop all related email notifications.
